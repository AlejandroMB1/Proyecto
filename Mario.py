...
class Game:
	...
	def menu(self):
		...
		btnDificultad = Button(self.v1, text="DIFICULTAD", command=self.dificultadv, font=("Agency FB", 14), width=15,
						  height=1).place(
			x=230, y=330)

		btnControles = Button(self.v1, text="CONTROLES", command=self.controles, font=("Agency FB", 14), width=15,
							  height=1).place(x=230, y=280)
		btnSalir = Button(self.v1, text="SALIR", command=self.salir, font=("Agency FB", 14), width=15, height=1).place(
	...
	def dificultadv(self):
		...
		self.canvas = Canvas(self.newWindow2, width=500, height=450)  # tamaño ventana
		...
		btnFacil = Button(self.newWindow2, text="Facil", command=self.dificultadelegir1, font=("Agency FB", 14), width=10,
						 height=1).place(x=200, y=100)
		btnNormal = Button(self.newWindow2, text="Normal", command=self.dificultadelegir2, font=("Agency FB", 14),
						 width=10,
						 height=1).place(x=200, y=200)
		btnDificultad= Button(self.newWindow2, text="Dificil", command=self.dificultadelegir3, font=("Agency FB", 14),
						 width=10,
	...
	def controles(self):
		...
		btnBack = Button(self.newWindow, text="BACK", command=self.quitarco, font=("Agency FB", 14), width=10, height=1).place(x=450, y=380)
	...
	def comenzarJuego(self):
		...
		self.luigi = self.canvas.create_text(400, 10, anchor="nw", fill="red")
		self.canvas.itemconfig(self.luigi, text="Luigi")
		
		

		self.mario2 = self.canvas.create_text(140, 25, anchor="nw", fill="red")
		self.canvas.itemconfig(self.mario2, text=str(self.jugadores[0].vidas) + " vidas")
		self.luigi2 = self.canvas.create_text(400, 25, anchor="nw", fill="red")
		...
		self.canvas.itemconfig(self.mario3, text=str(self.jugadores[0].puntos) + " puntos")
		self.luigi3 = self.canvas.create_text(400, 40, anchor="nw", fill="red")
		...
		Button(self.v2, text="guardar", command=self.guardarjuego,font=("Agency FB", 12), width=8, height=1).place(
			x=250, y=9)
		Button(self.v2, text="cargar", command=self.cargarjuego, font=("Agency FB", 12), width=8, height=1).place(
			x=320, y=9)
		self.detectarmuerte()
	...
	def detectarnivel(self):
		...
		if self.enemigos == []:
			if self.nivel == 1:
				...
				self.canvas.itemconfig(self.niveltexto, text="YOU WIN!")
	...
	def nivel3(self):
		...
		self.canvas.after(12000, self.eliminarpowerup)
	...
	def nivel4(self):
		...
		self.canvas.after(5000, self.eliminarpowerup)
	...
	def nivel5(self):
		...
		self.powerupimg = PhotoImage(file="img/powerup.png").zoom(2)
		#añado el powerup
		self.canvas.after(3000, self.aparecerpowerup)
		self.canvas.after(7000, self.eliminarpowerup)
